...
public sealed class ShopSyncCreateCommandValidator : AbstractValidator<ShopSyncCreateCommand>
{
    ...
    private void AddShopRules(
        System.Linq.Expressions.Expression<Func<ShopSyncCreateCommand, SMIS.Application.DTO.Shops.ShopSyncCreateDto>>
            dto
    )
    {
        RuleFor(dto).ChildRules(rules =>
        {
            ...
            rules.RuleFor(x => x.PhoneNumber).MinimumLength(8).MaximumLength(20).Matches(@"^\+?[\d\s\-()]+$")
                .When(x => !string.IsNullOrEmpty(x.PhoneNumber));
            rules.RuleFor(x => x.Email).EmailAddress().MaximumLength(100).When(x => !string.IsNullOrEmpty(x.Email));
            rules.RuleFor(x => x.TaxNumber).MinimumLength(5).MaximumLength(20).Matches(@"^[A-Z0-9\-]+$")
...
public sealed class ShopSyncUpdateCommandValidator : AbstractValidator<ShopSyncUpdateCommand>
{
    public ShopSyncUpdateCommandValidator()
    {
        ...
        RuleFor(x => x.Dto.PhoneNumber).MinimumLength(8).MaximumLength(20).Matches(@"^\+?[\d\s\-()]+$")
        ...
        RuleFor(x => x.Dto.TaxNumber).MinimumLength(5).MaximumLength(20).Matches(@"^[A-Z0-9\-]+$")
            .When(x => !string.IsNullOrEmpty(x.Dto.TaxNumber));
