...
namespace SMIS.Application.Features.Provinces.Queries
{
    ...
    internal sealed class ProvinceTranslationGetListQueryHandler : IRequestHandler<ProvinceTranslationGetListQuery,
        Result<List<ProvinceTranslationDto>>>
    {
        private readonly IProvinceRepository _repo;
        private readonly IMapper _mapper;

        public ProvinceTranslationGetListQueryHandler(
        ...
        public async Task<Result<List<ProvinceTranslationDto>>> Handle(
            ProvinceTranslationGetListQuery request,
            CancellationToken cancellationToken
        )
        {
            var province = await _repo.GetFirstOrDefaultAsync(x => x.Id == request.ProvinceId,
                includeProperties: $"{nameof(Province.Translations)}.{nameof(ProvinceTranslation.Language)}");
