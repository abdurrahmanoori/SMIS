...
{
    public record GetUnitOfMeasureListQuery(int PageNumber = 1, int PageSize = 25) : IRequest<Result<PagedList<UnitOfMeasureDto>>>;

    internal sealed class GetUnitOfMeasureListQueryHandler : IRequestHandler<GetUnitOfMeasureListQuery, Result<PagedList<UnitOfMeasureDto>>>
    {
        private readonly IUnitOfMeasureRepository _unitOfMeasureRepository;
        private readonly IMapper _mapper;

        public GetUnitOfMeasureListQueryHandler(IUnitOfMeasureRepository unitOfMeasureRepository, IMapper mapper)
        {
            _unitOfMeasureRepository = unitOfMeasureRepository;
            _mapper = mapper;
        }

        public async Task<Result<PagedList<UnitOfMeasureDto>>> Handle(GetUnitOfMeasureListQuery request, CancellationToken cancellationToken)
        {
