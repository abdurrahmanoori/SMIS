...
{
    public static string Id(string value) => Guid.Parse(value).ToString("D"); public static bool User(string? value, ICurrentUser user) => string.IsNullOrWhiteSpace(value) || string.Equals(value.Trim(), user.GetId(), StringComparison.Ordinal);
    ...
    public static Result<UnitOfMeasureDto> InvalidUser() => Result<UnitOfMeasureDto>.FailureResult("InvalidClientUser", "Client user metadata must match the authenticated user.");
}
