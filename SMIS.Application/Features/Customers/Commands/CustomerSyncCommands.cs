...
internal sealed class CustomerSyncCreateCommandHandler : IRequestHandler<CustomerSyncCreateCommand, Result<CustomerDto>>
{
    ...
    public CustomerSyncCreateCommandHandler(
    ...
    public async Task<Result<CustomerDto>> Handle(
        CustomerSyncCreateCommand request,
        CancellationToken cancellationToken
    )
    {
        var id = CustomerSyncRules.NormalizeGuid(request.Dto.Id);
        if (!CustomerSyncRules.UserMatches(request.Dto.ClientCreatedBy, _currentUser) ||
            !CustomerSyncRules.UserMatches(request.Dto.ClientModifiedBy, _currentUser))
            return CustomerSyncRules.InvalidUser();
        ...
        var customer = Customer.Create(request.Dto.FirstName, _currentUser.GetShopId(), request.Dto.CustomerType,
            request.Dto.LastName, request.Dto.FatherName, request.Dto.Email, request.Dto.PhoneNumber,
            request.Dto.Address, request.Dto.TaxNumber, request.Dto.ProvinceId, request.Dto.DistrictId,
...
internal sealed class CustomerSyncUpdateCommandHandler : IRequestHandler<CustomerSyncUpdateCommand, Result<CustomerDto>>
{
    ...
    public CustomerSyncUpdateCommandHandler(
        ICustomerRepository repository,
        IUnitOfWork unitOfWork,
        ICurrentUser currentUser,
        IMapper mapper
    ...
    public async Task<Result<CustomerDto>> Handle(
        CustomerSyncUpdateCommand request,
        CancellationToken cancellationToken
    )
    {
        if (!CustomerSyncRules.UserMatches(request.Dto.ClientModifiedBy, _currentUser))
            return CustomerSyncRules.InvalidUser();
        var customer =
            await _repository.GetByIdIncludingDeletedAsync(CustomerSyncRules.NormalizeGuid(request.Id),
                cancellationToken);
        ...
        var modified = DateTimeService.NormalizeUtc(request.Dto.ClientModifiedDate);
        if (modified <= customer.GetConflictModifiedUtc())
            return Result<CustomerDto>.SuccessResult(_mapper.Map<CustomerDto>(customer));
        CustomerSyncRules.Apply(customer, request.Dto);
        customer.SetClientModificationMetadata(modified, request.Dto.ClientModifiedBy);
        customer.Restore();
        await _unitOfWork.SaveChanges(cancellationToken);
        return Result<CustomerDto>.SuccessResult(_mapper.Map<CustomerDto>(customer));
...
internal sealed class CustomerSyncDeleteCommandHandler : IRequestHandler<CustomerSyncDeleteCommand, Result<CustomerDto>>
{
    ...
    public CustomerSyncDeleteCommandHandler(
        ICustomerRepository repository,
        IUnitOfWork unitOfWork,
        ICurrentUser currentUser,
        IMapper mapper
    ...
    public async Task<Result<CustomerDto>> Handle(
        CustomerSyncDeleteCommand request,
        CancellationToken cancellationToken
    )
    {
        if (!CustomerSyncRules.UserMatches(request.Dto.ClientModifiedBy, _currentUser))
...
internal static class CustomerSyncRules
{
    ...
    public static bool CanAccess(
        Customer customer,
        ICurrentUser currentUser
    ) => customer.ShopId == currentUser.GetShopId();
    ...
    )
    {
        ...
        customer.SetFatherName(dto.FatherName);
        customer.SetEmail(dto.Email);
        customer.SetPhoneNumber(dto.PhoneNumber);
        customer.SetAddress(dto.Address);
        customer.SetTaxNumber(dto.TaxNumber);
        customer.SetProvinceId(dto.ProvinceId);
        customer.SetDistrictId(dto.DistrictId);
        if (dto.IsActive) customer.Activate();
        else customer.Deactivate();
    }

    public static Result<CustomerDto> InvalidUser() => Result<CustomerDto>.FailureResult("InvalidClientUser",
        "Client user metadata must match the authenticated user.");

    public static Result<CustomerDto> Forbidden() =>
        Result<CustomerDto>.FailureResult("Forbidden", "You can only synchronize customers from your own shop.");
}