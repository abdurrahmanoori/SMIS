...
public record LoanAccountGetListQuery(
    int PageNumber = 1,
    int PageSize = 25,
    bool IncludeCustomer = false,
    bool IncludeSale = false
) : IRequest<Result<PagedList<LoanAccountDto>>>;

internal sealed class
    LoanAccountGetListQueryHandler : IRequestHandler<LoanAccountGetListQuery, Result<PagedList<LoanAccountDto>>>
{
    private readonly ILoanAccountRepository _loanAccountRepository;
    private readonly IMapper _mapper;

    public LoanAccountGetListQueryHandler(
    ...
    public async Task<Result<PagedList<LoanAccountDto>>> Handle(
        LoanAccountGetListQuery request,
    ...
    )
    {
        ...
        var query = _loanAccountRepository.GetAllQueryable(
