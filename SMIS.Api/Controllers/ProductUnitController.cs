...
namespace SMIS.Api.Controllers
{
    ...
    public class ProductUnitController : BaseApiController
    {
        ...
        [HttpPost("sync")]
        public async Task<ActionResult<ProductUnitDto>> SyncCreate(ProductUnitSyncCreateDto dto) => HandleResultResponseOld(await Mediator.Send(new ProductUnitSyncCreateCommand(dto)));
        ...
        [HttpPut("{id}/sync")]
        public async Task<ActionResult<ProductUnitDto>> SyncUpdate(string id, ProductUnitSyncUpdateDto dto) => HandleResultResponseOld(await Mediator.Send(new ProductUnitSyncUpdateCommand(id, dto)));
        ...
        [HttpDelete("{id}/sync")]
        public async Task<ActionResult<ProductUnitDto>> SyncDelete(string id, ProductUnitSyncDeleteDto dto) => HandleResultResponseOld(await Mediator.Send(new ProductUnitSyncDeleteCommand(id, dto)));

        [HttpGet("pull")]
        public async Task<ActionResult<List<ProductUnitDto>>> Pull([FromQuery] DateTime changedSince) => HandleResultResponseOld(await Mediator.Send(new ProductUnitPullQuery(changedSince)));
    }
}
