...
class ProductRepository {
  ...
  Future<Product> update(String id, ProductDraft draft) async {
    ...
    final remainsCreate =
        existing.pendingOperation == ProductPendingOperation.create;
    final record = existing.copyWith(
      ...
      pendingOperation: remainsCreate
          ? ProductPendingOperation.create
          : ProductPendingOperation.update,
  ...
  Future<List<ProductLocalRecord>> getPendingRecords({
    ...
    final retryFilter = force
        ? ''
        : ' AND retry_count < 5 AND (next_retry_at IS NULL OR next_retry_at <= ?)';
    final rows = await database.query(
      _table,
  ...
  Future<void> _ensureProductReferences({
    required String shopId,
    ...
    final shop = await database.query(
      'shops',
      columns: ['id'],
    ...
    final unit = await database.query(
      'unit_of_measures',
      columns: ['id'],
      where: 'id = ? AND is_deleted = 0',
  ...
  Future<bool> tryAcquireSyncLock(String owner, String shopId) async {
    ...
    return database.transaction((transaction) async {
      ...
      if (rows.isNotEmpty) {
        final lockedAt = DateTime.tryParse(
